...
import pytest
from domain.model import OrderLine, Batch, OutOfStock, Product
...
def test_prefers_current_stock_batches_to_shipments():
    ...
    shipment_batch = Batch("shipment-bacth", "RETRO-CLOCK", 100, eta=tomorrow)
...
def test_prefers_earlier_batches():
    ...
    product = Product(sku="RETRO-CLOCK",
                      batches=[earliest, medium, latest])
...
def test_returns_allocated_batch_ref():
    in_stock_batch = Batch("in-stock-batch-ref", "HIGHBROW-POSTER", 100,
                           eta=None)
    shipment_batch = Batch("in-stock-batch-ref", "HIGHBROW-POSTER", 100,
                           eta=tomorrow)
...
def test_raises_out_of_stock_exception_if_cannot_allocate():
    ...
    product = Product(sku="SMALL-FORK",
                      batches=[batch])
