...
class TestAddBatch:
    ...
    def test_add_batch_for_existing_product(self):
        ...
        messagebus.handle(
            commands.CreateBatch("b1", "CRUNCHY-ARMCHAIR", 100, None), uow)
        messagebus.handle(
            commands.CreateBatch("b2", "CRUNCHY-ARMCHAIR", 10, None), uow)
...
class TestAllocate:
    ...
    def test_sends_email_on_out_of_stock_error(self):
        uow = FakeUnitOfWork()
        messagebus.handle(
            commands.CreateBatch("batch1", "COMPLICATED-LAMP", 9, None), uow)

        with mock.patch(
                "allocation.adapters.email.send_mail") as mock_send_email:
            messagebus.handle(
                commands.Allocate("01", "COMPLICATED-LAMP", 10), uow)
